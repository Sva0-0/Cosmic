            ...
            <VStack spacing={4} w='full' alignItems={'flex-start'}>
                <Flex w='full' gap='24px' direction={'column'}>
                    ...
                    <SimpleGrid columns={2} spacing={6}>
                        {keyList.map(key => (
                            <NextLink key={key} href={LINKS.home(key, chainId)} passHref legacyBehavior>
                                <GridItem as="a" px='4' py='3' rounded="xl" bg='gray.100' textTransform="uppercase" fontWeight='bold' cursor="pointer" _hover={{ bg: 'gray.200' }}>
                                    {key}
                                </GridItem>
                            </NextLink>
                        ))}
