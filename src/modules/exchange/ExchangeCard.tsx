...
const ExchangeCard: FC<ExchangeCardProps> = (props) => {
  ...
  // Helper to override visible name
  const displaySymbol = symbol === "AURA" ? "COSMIC" : symbol;
  ...
  return (
    <Box borderWidth='1px' borderRadius='lg' padding={6} data-testid="exchange-card">
      <Text fontWeight="bold" fontSize="3xl" data-testid="buy-tokens-title">Buy {displaySymbol} Tokens</Text>
      <Flex gap={6} backgroundColor={"blue.50"} rounded={"sm"} py={1} px={4} mt={6} data-testid="supply-info">
        <Flex gap={1}>
          ...
          <Text fontWeight={"bold"}>{formatNumber(total_amount)} {displaySymbol}</Text>
        </Flex>
        <Flex gap={1}>
          <Text color={"blackAlpha.600"}>Available for Purchase</Text>
          <Text color={"blackAlpha.600"} fontWeight={"bold"}>{formatNumber(amount)} {displaySymbol}</Text>
        </Flex>
      </Flex>
      <Box mt={6}>
        ...
        <Box data-testid="exchange-actions">
          <Box mb={6}>
            <Flex justify={"space-between"} mb={2}>
              <Text color={"blackAlpha.600"}>You get {displaySymbol}</Text>
            </Flex>
